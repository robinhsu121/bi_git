...
def hdfs_join_cms(cms_df):
    ...
    ##hdfs_df
    sql = """ 
    select t0.custom_uuid,t0.date,t0.package_id,t0.title,t0.site,t0.device_name from sharp.download_fin t0 where t0.dt="{date_0}" 
    union all
    select t1.custom_uuid,t1.date,t1.package_id,t1.title,t1.site,t1.device_name from sharp.download_fin t1 where t1.dt="{date_1}"
    union all
    select t7.custom_uuid,t7.date,t7.package_id,t7.title,t7.site,t7.device_name from sharp.download_fin t7 where t7.dt="{date_7}" """ .format(date_0=str_dt_0,date_1=str_dt_1,date_7=str_dt_7)
    spark.sql("show databases")
    spark.sql("use sharp")
    ...
    df = hdfs_df.alias("t_0").join(cms_df.alias("t_1"),condition_0_1,"left_outer") \
                             .select(F.col("t_0.custom_uuid").alias("custom_uuid"),F.col("t_0.date").alias("date"),F.col("t_0.site").alias("site"),F.col("t_0.device_name").alias("device_name"),F.col("t_0.package_id").alias("package_id"), \
...
def app_download(df):
    ...
    app_report = df_cube_0.alias("t_0").join(df_cube_1.alias("t_1"),conditions_0_1,"left_outer" ) \
                                       ...
                                       .select(F.regexp_replace(F.lit(str_dt_0),"-","").cast("int").alias("date"),F.col("t_0.package_id").alias("appId"),F.col("t_0.title").alias("appName"),F.col("t_0.site").alias("channelName"),F.col("t_0.fsk_catalog").alias("typeName"), \
...
##机型下载量函数(增加内容方、应用类型汇总2个维度)
def device_download_(df):
    """各机型下载量报表函数"""
    #分析数据
    df.createOrReplaceTempView("v_df")
    sql = """ select cast(regexp_replace(date,'-','') as int) date,device_name,site channelName,fsk_catalog typeName,grouping_id() id_1,count(custom_uuid) totalPlayCount from v_df where date="{date_0}" group by cast(regexp_replace(date,'-','') as int),device_name,site,fsk_catalog grouping sets((cast(regexp_replace(date,'-','') as int),device_name,site,fsk_catalog),(cast(regexp_replace(date,'-','') as int),device_name,site),(cast(regexp_replace(date,'-','') as int),device_name,fsk_catalog),(cast(regexp_replace(date,'-','') as int),device_name)) """.format(date_0=str_dt_0)
    spark.sql("show databases")
    spark.sql("use sharp")
    df_cube_0 = spark.sql(sql)
    return df_cube_0
...
def main():
    ...
    gc.collect()
    ##每日机型下载量报表_改进
    device_report_ = device_download_(df_stat)
    hdfs_to_mysql(device_report_,"device_app_download_report_","append")
    del device_report_
