...
spark = SparkSession.builder.master("spark://master:7077").appName("app_game_open_report").enableHiveSupport().getOrCreate()
...
##设定日期
#today=datetime.date(2019,03,27)
...
##解析 app_open & game_open 日志函数
def parse_app_game_open():
    """解析video_play_count日志数据"""
    ##json日志文件的路径
    ...
    if int(if_zero) == 0:
        ...
        #json日志数据路径,并解析
        df_app=spark.read.json(json_path).select('custom_uuid','rectime','device_name','vercode','vername','site',F.explode('data.app_open').alias('app_open')).filter(F.col("app_open.time").isNotNull()).select(['custom_uuid',F.to_date(F.from_unixtime(F.col('rectime')/1000)).cast('date').alias("date"),F.to_date(F.from_unixtime(F.col('rectime')/1000)).cast('string').alias('dt'),'device_name','vercode','vername',F.when(F.col('site') == 'ALI','youku').when(F.col('site') == 'IQIYI','iqiyi').when(F.col('site') == 'BESTV','bestv').otherwise('others').alias('site'),F.col('app_open.package_id').alias('package_id'),F.col('app_open.title').alias('title'),F.when(F.col('app_open.package_id').isNotNull(),1).otherwise(0).alias('source')])
        df_game=spark.read.json(json_path).select('custom_uuid','rectime','device_name','vercode','vername','site',F.explode('data.game_open').alias('game_open')).filter(F.col("game_open.time").isNotNull()).select(['custom_uuid',F.to_date(F.from_unixtime(F.col('rectime')/1000)).cast('date').alias("date"),F.to_date(F.from_unixtime(F.col('rectime')/1000)).cast('string').alias('dt'),'device_name','vercode','vername',F.when(F.col('site') == 'ALI','youku').when(F.col('site') == 'IQIYI','iqiyi').when(F.col('site') == 'BESTV','bestv').otherwise('others').alias('site'),F.col('game_open.package_id').alias('package_id'),F.col('game_open.title').alias('title'),F.when(F.col('game_open.package_id').isNotNull(),1).otherwise(0).alias('source')])
        ##二者综合起来
        df_stat=df_app.unionAll(df_game)
        ##把数据插入hive动态分区表中
        spark.sql("set hive.exec.dynamic.partition=true")
        spark.sql("set hive.exec.dynamic.partition.mode=nonstrict")
        spark.sql("show databases")
        spark.sql("use sharp")
        df_stat.write.format("orc").mode("append").partitionBy("dt").saveAsTable("sharp.app_open")


##HDFS:app_open&game_open与CMS数据库连接
def hdfs_join_cms(cms_df):
    ...
    union all
    select t7.custom_uuid,t7.date,t7.package_id,t7.title,t7.site,t7.source from sharp.app_open t7 where t7.dt="{date_7}" """ .format(date_0=str_dt_0,date_1=str_dt_1,date_7=str_dt_7)
    spark.sql("show databases")
    spark.sql("use sharp")
    ...
    df = hdfs_df.alias("t_0").join(cms_df.alias("t_1"),condition_0_1,"left_outer") \
                                 ...
                                 F.col("t_0.title").alias("title_1"),F.col("t_1.fsk_title").alias("title_2"),F.col("t_0.source").alias("source"),F.col("t_1.fsk_catalog").alias("fsk_catalog")) \
                                 .select(["custom_uuid","date","site","package_id",F.when(F.col("package_id").isNotNull(),F.col("title_2")).otherwise(F.col("title_1")).alias("title"),"source","fsk_catalog"])
    return df
...
def app_open(df):
    ...
    sql_0 = """select package_id,title,source,site,fsk_catalog,grouping_id() id_1,count(custom_uuid) playNum,count(distinct custom_uuid) users,round(count(custom_uuid)/count(distinct custom_uuid),2) avgPlayNum from v_df where date="{date_0}" group by package_id,title,source,site,fsk_catalog  grouping sets((package_id,title,source,site,fsk_catalog),(source,site,fsk_catalog),(source,site),(source,fsk_catalog),(site,fsk_catalog),(source),(site),(fsk_catalog),()) """.format(date_0=str_dt_0)
    sql_1 = """select package_id,title,source,site,fsk_catalog,grouping_id() id_1,count(custom_uuid) playNum,count(distinct custom_uuid) users,round(count(custom_uuid)/count(distinct custom_uuid),2) avgPlayNum from v_df where date="{date_1}" group by package_id,title,source,site,fsk_catalog  grouping sets((package_id,title,source,site,fsk_catalog),(source,site,fsk_catalog),(source,site),(source,fsk_catalog),(site,fsk_catalog),(source),(site),(fsk_catalog),()) """.format(date_1=str_dt_1)
    sql_7 = """select package_id,title,source,site,fsk_catalog,grouping_id() id_1,count(custom_uuid) playNum,count(distinct custom_uuid) users,round(count(custom_uuid)/count(distinct custom_uuid),2) avgPlayNum from v_df where date="{date_7}" group by package_id,title,source,site,fsk_catalog  grouping sets((package_id,title,source,site,fsk_catalog),(source,site,fsk_catalog),(source,site),(source,fsk_catalog),(site,fsk_catalog),(source),(site),(fsk_catalog),()) """.format(date_7=str_dt_7)
    spark.sql("show databases")
    spark.sql("use sharp")
    ...
    condition_2=(F.coalesce(F.col("t_0.source"),F.lit("123")) == F.coalesce(F.col("t_1.source"),F.lit("123"))) 
    condition_3=(F.coalesce(F.col("t_0.site"),F.lit("123")) == F.coalesce(F.col("t_1.site"),F.lit("123")))  
    condition_4=(F.coalesce(F.col("t_0.fsk_catalog"),F.lit("123")) == F.coalesce(F.col("t_1.fsk_catalog"),F.lit("123"))) 
    condition_5=(F.col("t_0.id_1") == F.col("t_1.id_1")) 
    condition_6=(F.coalesce(F.col("t_0.package_id"),F.lit("123")) == F.coalesce(F.col("t_7.package_id"),F.lit("123"))) 
    condition_7=(F.coalesce(F.col("t_0.title"),F.lit("123")) == F.coalesce(F.col("t_7.title"),F.lit("123"))) 
    condition_8=(F.coalesce(F.col("t_0.source"),F.lit("123")) == F.coalesce(F.col("t_7.source"),F.lit("123"))) 
    condition_9=(F.coalesce(F.col("t_0.site"),F.lit("123")) == F.coalesce(F.col("t_7.site"),F.lit("123"))) 
    condition_10=(F.coalesce(F.col("t_0.fsk_catalog"),F.lit("123")) == F.coalesce(F.col("t_7.fsk_catalog"),F.lit("123"))) 
    ...
    app_report = df_cube_0.alias("t_0").join(df_cube_1.alias("t_1"),conditions_0_1,"left_outer" ) \
                                       ...
                                       .select(F.regexp_replace(F.lit(str_dt_0),"-","").cast("int").alias("date"),F.col("t_0.package_id").alias("appId"),F.col("t_0.title").alias("appName"),F.col("t_0.source").alias("appSource"),F.col("t_0.site").alias("channelName"),F.col("t_0.fsk_catalog").alias("typeName"),F.col("t_0.id_1").alias("id_1"), \
...
def main():
    ...
    #分析形成报表并写入mysql
    app_report = app_open(df_stat)
    hdfs_to_mysql(app_report,"app_open_report","append")
    del app_report
    gc.collect()
    

if  __name__ == "__main__":
    """ app_open_report 报表入口 """
    import time
