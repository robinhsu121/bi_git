...
def cms_to_hdfs():
    """ 将数据mysql导入hdfs"""
    table_name = """ (select fsk_pid,fsk_title,fsk_cid,fsk_catalog  from fsk.fsk_app_list union all select fsk_pid,fsk_title,fsk_cid,fsk_catalog  from fsk.fsk_game_list) as fsk_app_list """
...
def hdfs_join_cms(cms_df):
    ...
    df = hdfs_df.alias("t_0").join(cms_df.alias("t_1"),condition_0_1,"left_outer") \
                                 ...
                                 F.col("t_0.title").alias("title_1"),F.col("t_1.fsk_title").alias("title_2"),F.col("t_0.source").alias("source"),F.col("t_1.fsk_cid").alias("fsk_cid")) \
                                 .select(["custom_uuid","date","site","package_id",F.when(F.col("package_id").isNotNull(),F.col("title_2")).otherwise(F.col("title_1")).alias("title"),"source","fsk_cid"])
    return df
...
def app_open(df):
    ...
    condition_4=(F.coalesce(F.col("t_0.fsk_cid"),F.lit("123")) == F.coalesce(F.col("t_1.fsk_cid"),F.lit("123"))) 
    ...
    condition_10=(F.coalesce(F.col("t_0.fsk_cid"),F.lit("123")) == F.coalesce(F.col("t_7.fsk_cid"),F.lit("123"))) 
...
def main():
    ...
    hdfs_to_mysql(app_report,"app_open_report_","append")
    del app_report
    gc.collect()
...
if  __name__ == "__main__":
    ...
    begin = datetime.date(2019,01,03)
    end = datetime.date(2019,02,01)
    d = begin
